...
- **Maschinenlesbare Upgrade-Flächen:** `system/hub/upgrade.py` liefert für `bach upgrade list/status/check --json` jetzt strukturierte Versions-, Release- und Drift-Payloads für Automationen und Dashboards.
...
- **Heutige Release-Verifikation nachgezogen:** README, README.de, ROADMAP und `.dev/NEXT_RELEASE.md` spiegeln jetzt die Live-Smokes vom 2026-05-22 (`--startup quick`, `agent doctor/start/pause/resume/steer/status`, `scheduler doctor`, `usecase run 12/41`, `lang report --surface gui`, `upgrade check --json`) sowie den aktualisierten OpenClaw-Referenzstand.
...
- **Agent-Hilfe und Release-Planung nachgezogen:** Hilfe, README, ROADMAP, CHANGELOG und `NEXT_RELEASE` dokumentieren jetzt die Vorstart-Steuerung mit `queued_for_next_start`, `status=queued`, Queue-Persistenz über den nächsten Agentenstart, die direkte `CLAUDE.md`-Injection sowie kooperative `pause`/`resume`-Anfragen mit verschachteltem `operator_control`.
- **Upgrade-Hilfe und Root-READMEs nachgezogen:** Hilfe, README und README.de dokumentieren jetzt `bach upgrade list/status/check --json` als maschinenlesbare Drift- und Release-Fläche für Automationen.
...
- **OpenClaw-Referenzstand aktualisiert:** Doku und Release-Planung erfassen jetzt den verifizierten Stand vom 2026-05-22 mit GitHub-Stable `2026.5.20`, sichtbarem Prerelease `2026.5.20-beta.2`, stabiler `2026.5.20-slim`-Containerfamilie, zusätzlicher Alpha-Linie `2026.5.21-alpha.1-slim` und den daraus erneut bestätigten Beobachtungspunkten rund um Exec-Approvals, Policy-/Doctor-Lints, Aufgabenwartungs-JSON, Plaintext-Secret-Warnungen und längere Codex-/Image-Watchdogs.
