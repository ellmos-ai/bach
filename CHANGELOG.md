...
- **Heutige Release-Verifikation nachgezogen:** README, README.de, ROADMAP und `.dev/NEXT_RELEASE.md` spiegeln jetzt die Live-Smokes vom 2026-05-30 (`bach --startup quick --mode=silent --partner=codex`, `bach upgrade repair/status/check --json`, `bach task list`, `bach agent doctor test-agent --json`, kompletter `test-agent`-Headless-Lauf, `usecase run-all --dry-run`, `usecase run 12/41/45 --dry-run`, `bach lang report --surface gui --limit 5 --json`) sowie die gezielten Pytest-Suiten für Upgrade-Kontrollflächen. Gleichzeitig wurde der OpenClaw-Referenzstand auf Stable `2026.5.27`, sichtbares Release-Prerelease `2026.5.28-beta.4` und die aktuell hervorgehobene Containerlinie `2026.5.28-beta.4-slim` nachgezogen.
