...
- **Heutige Release-Verifikation nachgezogen:** README, README.de, ROADMAP und `.dev/NEXT_RELEASE.md` spiegeln jetzt die Live-Smokes vom 2026-05-23 (`--startup quick`, `agent doctor/start/pause/resume/checkpoint/steer/status`, `scheduler doctor`, `usecase run 12/41`, `usecase run-all --dry-run`, `lang report --surface gui`, `upgrade check --json`) sowie den präzisierten OpenClaw-Referenzstand mit Stable `2026.5.20`, sichtbarem Release-Prerelease `2026.5.20-beta.2` und vorgezogener Paketlinie `2026.5.22-beta.1-slim`.
