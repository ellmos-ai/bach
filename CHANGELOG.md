...
- **Gemeinsames Gedächtnisschema, Vertrag v2 mit Lesson-Schema v2 (S2a, T-20260920-823767362):** Vertrag und Modul erneut byte-identisch aus `ellmos-ai/usmc` übernommen (neue SHA-Pins). `043_memory_union.py` ergänzt `memory_lessons` additiv um Provenienz-, Idempotenz-, Feedback- und Zustellspalten und legt `memory_lesson_feedback`, `memory_lesson_delivery_batches` und `memory_lesson_deliveries` an. Gegen eine Kopie der Laptop-`bach.db` gemessen: 0,5 s, Zeilen und ID-Summen aller sechs Gedächtnistabellen gleich, 41 blockierte Trigger erhalten, `integrity_check` ok, zweiter Lauf No-op. Weiterhin **nicht in `migrations/`**; die Aktivierung folgt als eigener Schritt (S2b).
