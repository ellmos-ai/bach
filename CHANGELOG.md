...
- **OpenClaw-Referenzstand aktualisiert:** Doku und Release-Planung erfassen jetzt den verifizierten Stand vom 2026-05-15 mit GitHub-Stable `2026.5.12`, sichtbarem Prerelease `2026.5.14-beta.1`, `latest`-Containerlinie `2026.5.12-slim` und der GHCR-Versionsseite `2026.5.14-beta.2-slim`; zusätzlich schärfen die jüngsten offiziellen Hinweise ACP-Fallbacks, SecretRef-only Credentials, `USERPROFILE`-Sandbox-Härtung, Auth-Flag-Weitergabe, Startup-Trace-Attribution sowie reproduzierbare Docker-Validierungslanes als relevante Beobachtungspunkte für BACH.
