...
Stand 2026-07-11: Auf der OpenClaw-Releases-Seite bleibt `2026.6.11` die aktuelle Stable-Linie; das neueste sichtbare Prerelease ist jetzt `2026.7.1-beta.5`, veröffentlicht am 2026-07-11 um 10:36 UTC. Für BACH bleiben daraus vor allem GPT-5.6-Modellfamilien-Support, `openclaw attach` für externe Harness-Anbindung an bestehende Sessions, Telegram-Codex-Pairing/Steering/Recovery, exit-getriggerte Cron-Läufe mit sauberem Detach, gescopte Conversation-Capability-Profile sowie deutlichere Doctor-/Install-Diagnostik relevant. Die neueste Beta ergänzt zusätzlich conversational onboarding mit modellgeführten Freigaben, einen gebündelten ClawRouter-Provider mit credential-scoped Modell-Discovery samt Usage-/Budget-Sicht, robustere Crash-Loop-Recovery im Gateway und stärkere Session-Organisation. Für BACH passen davon besonders approval-geführte Setup-Flows, Provider-/Modell-Discovery mit Budgetsicht und recovery-sichere Runtime-/Scheduler-Oberflächen. Quelle: [openclaw/openclaw releases](https://github.com/openclaw/openclaw/releases).

Auf BACH-Seite bleibt der saubere GUI-i18n-Stand vom 2026-06-12 mit 17.593 exportierten Übersetzungen und 0 offenen GUI-Einträgen bestehen; der aktive Workflow-Katalog zählt weiterhin konsistent 71 Vorlagen. Der Daily-Care-Lauf vom 2026-07-11 hat den Runtime-Compat-Slice für Task `1154` geschlossen: `clutch`-gestützte Adapter decken jetzt Streckenanalyse, Gas/Bremse und die Bordcomputer-Health-Fläche ab, behalten BACH-Signaturen und den kanonischen `bach.db`-Pfad bei, und `bach clutch migration` meldet dadurch nun `Compat-Adapter: OK` sowie `DB-Brücke: OK`, während die Fork-Archivierung bewusst weiter blockiert bleibt. Im selben Lauf wurden `python -m pytest system/tests/test_delegation_adapter.py system/tests/test_clutch.py system/tests/test_partner_handler.py -q`, `python bach.py partner delegate "Externe clutch Adapter fuer Streckenanalyse, Gas Bremse und Bordcomputer pruefen" --score` und `python bach.py clutch analyse "Externe clutch Adapter fuer Streckenanalyse, Gas Bremse und Bordcomputer pruefen"` erneut verifiziert. Nächste Schritte bleiben tieferes Active-Run-Steering in langlaufenden Scheduler-Innenschleifen, Installer-End-to-End-, GUI-Regressions- sowie die breiteren T01/T02-Retestspuren.
