...
Stand 2026-06-21: Auf der OpenClaw-Releases-Seite ist die Stable-Linie jetzt `2026.6.9`; die sichtbaren Release-Artefakte und Source-Archive tragen Zeitstempel zwischen `2026-06-21T01:16:53Z` und `2026-06-21T03:57:38Z`. Das neueste sichtbare Prerelease ist `2026.6.10-beta.1`, veröffentlicht am 2026-06-21 um 09:12 UTC. Für BACH sind daraus vor allem reichhaltigere Telegram-/Slack-/Discord-Zustellung, robustere Agent-/Session-Recovery, stärkere Codex-Freigabe- und Plugin-Pfade, eigenständige Provider-Plugin-Pakete, explizite Such-/Skill-Herkunft sowie sicherere Release-/Netzgrenzen relevant. Quelle: [openclaw/openclaw releases](https://github.com/openclaw/openclaw/releases).

Auf BACH-Seite bleibt der saubere GUI-i18n-Stand vom 2026-06-12 mit 17.593 exportierten Übersetzungen und 0 offenen GUI-Einträgen bestehen; der aktive Workflow-Katalog zählt weiterhin konsistent 71 Vorlagen. Der Daily-Care-Lauf vom 2026-06-21 härtet jetzt zusätzlich den ProSync-Startup-Pfad: `system/hub/db_sync.py` staged OneDrive-Transit-Backups vor dem Merge lokal und verschiebt einen fehlschlagenden Backup-Kandidaten nach Cloud-Timeout- oder `disk I/O`-Fehlern für rund 30 Minuten, damit ein einzelnes unlesbares `.bachdb`-Artefakt nicht mehr jede CLI- oder `--json`-Abfrage erneut ausbremst. Verifiziert wurden `python -m pytest system/tests/test_db_sync_handler.py -q` (`45 passed`), `python -m pytest system/tests/test_prosync_race.py -q` (`3 passed`), ein real reproduzierter OneDrive-Timeout im direkten `sync_on_start()`, danach `python bach.py agent doctor test-agent --json`, `python bach.py --startup quick --mode=silent --partner=codex`, `python bach.py usecase run 50 --dry-run`, `python bach.py upgrade check --json`, `python bach.py task list --filter clutch` sowie der vollständige `test-agent`-Steuerzyklus. Nächste Schritte bleiben tieferes Active-Run-Steering in langlaufenden Scheduler-Innenschleifen, Installer-End-to-End-, GUI-Regressions- sowie die breiteren T01/T02-Retestspuren.
