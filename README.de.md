...
Stand 2026-07-05: Auf der OpenClaw-Releases-Seite bleibt `2026.6.11` die aktuelle Stable-Linie; das neueste sichtbare Prerelease ist jetzt `2026.7.1-beta.2`, veröffentlicht am 2026-07-05 um 09:10 UTC. Für BACH sind daraus weiter vor allem GPT-5.6-Modellfamilien-Support, `openclaw attach` für externe Harness-Anbindung an bestehende Sessions, Telegram-Codex-Pairing/Steering/Recovery, exit-getriggerte Cron-Läufe mit sauberem Detach, gescopte Conversation-Capability-Profile sowie deutlichere Doctor-/Install-Diagnostik relevant; die neueste Beta ergänzt dafür konkretere ClawRouter-gestützte Modell-Discovery und robustere Recovery bei Device-Approvals und Plugin-Installationen. Das bestätigt BACHs Fokus auf Codex-/Runtime-Kontrolle, Scheduler-Trigger, Capability-Grenzen, Provider-/Modell-Discovery und belastbare Recovery-Oberflächen. Quelle: [openclaw/openclaw releases](https://github.com/openclaw/openclaw/releases).

Auf BACH-Seite bleibt der saubere GUI-i18n-Stand vom 2026-06-12 mit 17.593 exportierten Übersetzungen und 0 offenen GUI-Einträgen bestehen; der aktive Workflow-Katalog zählt weiterhin konsistent 71 Vorlagen. Der Daily-Care-Lauf vom 2026-07-05 hat den PartnerRegistry-Compat-Slice für Task `1151` geschlossen: `clutch`-gestütztes Partnerrouting wird jetzt aus BACH `partner_recognition` und `delegation_rules` gespeist, sodass sowohl Auto-Routing als auch explizite `--to=`-Delegationen BACH-Allow-Lists und clutch-Zonenökonomie respektieren und dabei die aktive Routing-Quelle sichtbar bleibt. Im selben Lauf wurden `partner delegate/list`, `agent doctor test-agent --json` und `usecase run 50 --dry-run` über den API-Pfad erneut verifiziert. Nächste Schritte bleiben tieferes Active-Run-Steering in langlaufenden Scheduler-Innenschleifen, Installer-End-to-End-, GUI-Regressions- sowie die breiteren T01/T02-Retestspuren.
