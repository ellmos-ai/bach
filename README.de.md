...
Stand 2026-05-15: GitHub Releases markiert `2026.5.12` als aktuelle Stable-Linie von OpenClaw und zeigt `2026.5.14-beta.1` als neuestes sichtbares Prerelease, während die GHCR-Versionsseite bereits `2026.5.14-beta.2-slim` listet; die stabile Container-Linie unter `latest` bleibt auf `2026.5.12-slim`, und die Paket-Startseite hinkt noch auf `2026.5.14-beta.1-slim`. Für BACH bleibt OpenClaw vor allem als Vergleichssystem für Plugin-Sicherheit, Laufzeitdiagnostik, Operator-Steuerung und Release-Härtung relevant. Die jüngsten offiziellen Hinweise schärfen insbesondere Provider-/Plugin-Externalisierung, ACP-Fallback-Backends, SecretRef-only Credential-Resolution, Windows-`USERPROFILE`-Sandbox-Härtung, provider-spezifische Auth-Flag-Weitergabe, Startup-Trace-Attribution, Mid-Run-Steering per `/queue steer`, explizite Gateway-Scopes für Browser-Steuerung, Heartbeat-Metadaten auf Agent-Events, garantierte Session-Erzeugung vor dem ersten Agent-zu-Agent-Send sowie package-installierte Docker-Validierungslanes. BACH deckt bereits manifest-first Plugin-Metadaten, fail-closed Setup-Guards für Shell/Desktop/MCP-Flächen, Scans mit Quarantäne vor der Installation von Skills/MCP-Servern/Plugins, Memory-/Wiki-Provenance-Ansichten, kanonische `bach path`-Oberflächen, maschinenlesbare Agent-/Scheduler-Statusflächen, `bach agent doctor` sowie `bach scheduler doctor` und `bach scheduler session doctor` als operatorseitige Startdiagnosen und workspace-scoped Cache-Invalidierung in der Agent-Runtime ab. Neu hinzugekommen sind stabilere Windows-Agenten-PIDs und maschinenlesbare `available_actions`-/`runtime_seconds`-Felder für Agenten- und Scheduler-Flächen. Nächste Schritte: Active-Run-Steering über llmauto-Ketten hinaus, Telemetrie und Release-Validierung, strengere Auth-Gates, breitere Recovery-/Startup-Hinweise und Fallback-Semantik dort, wo sie zu BACHs Multi-Partner-Architektur passt.
