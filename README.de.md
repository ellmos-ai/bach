...
Stand 2026-06-19: Das OpenClaw-Repository markiert weiterhin `2026.6.8`, veröffentlicht am 2026-06-16 um 16:32 UTC, als aktuelle Stable-Linie auf GitHub; das neueste sichtbare Prerelease auf der GitHub-Releases-Seite ist jetzt `2026.6.9-beta.1`, veröffentlicht am 2026-06-19 um 05:52 UTC. Für BACH sind daraus vor allem reichhaltigere Telegram-Zustellung, robustere Agent-/Session-Recovery nach unterbrochenen Turns, stärkere Codex-Integration mit automatischen Plugin-Freigaben, providerseitige Plugin-Pakete, bewusste Opt-in-Suche mit Herkunftsspur sowie härtere Secret-/Privacy-Scrubs relevant. Quellen: [openclaw/openclaw releases](https://github.com/openclaw/openclaw/releases), [openclaw/releases](https://github.com/openclaw/releases).

Auf BACH-Seite bleibt der saubere GUI-i18n-Stand vom 2026-06-12 mit 17.593 exportierten Übersetzungen und 0 offenen GUI-Einträgen bestehen; der aktive Workflow-Katalog zählt weiterhin konsistent 71 Vorlagen. Der Daily-Care-Lauf vom 2026-06-19 härtet zusätzlich `bach setup check`, indem die MCP-Installationen über `npm root -g` statt über langsamere `npm list`-Pfade geprüft werden, erweitert den Scheduler-Hintergrundstart für langsamere Windows-/OneDrive-PID-Dateien und verifiziert den Operator-Steuerpfad mit einem frischen `test-agent`-Headless-Zyklus. Verifiziert wurden `python -m pytest system/tests/test_setup_handler.py system/tests/test_scheduler_handler.py system/tests/test_bach_paths.py -q` (`232 passed`), `python -m pytest system/tests/test_self_heal_handlers.py -q -k startup_resource_summary_uses_current_layout_and_db_counts` (`1 passed`), `python bach.py setup check`, `python bach.py agent doctor test-agent --json`, der vollständige `test-agent`-Steuerzyklus, `python bach.py task list --filter clutch`, `python bach.py reflection status` sowie `python bach.py usecase run 50 --dry-run`. Nächste Schritte bleiben tieferes Active-Run-Steering in langlaufenden Scheduler-Innenschleifen, Installer-End-to-End-, GUI-Regressions- sowie die breiteren T01/T02-Retestspuren.
