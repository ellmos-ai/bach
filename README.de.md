...
Stand 2026-07-03: Auf der OpenClaw-Releases-Seite bleibt `2026.6.11` die aktuelle Stable-Linie; das neueste sichtbare Prerelease bleibt `2026.7.1-beta.1`, veröffentlicht am 2026-07-02 um 07:25 UTC. Für BACH sind daraus vor allem GPT-5.6-Modellfamilien-Support, `openclaw attach` für externe Harness-Anbindung an bestehende Sessions, Telegram-Codex-Pairing/Steering/Recovery, exit-getriggerte Cron-Läufe mit sauberem Detach, gescopte Conversation-Capability-Profile sowie deutlichere Doctor-/Install-Diagnostik relevant. Das bestätigt BACHs Fokus auf Codex-/Runtime-Kontrolle, Scheduler-Trigger, Capability-Grenzen und belastbare Recovery-Oberflächen. Quelle: [openclaw/openclaw releases](https://github.com/openclaw/openclaw/releases).

Auf BACH-Seite bleibt der saubere GUI-i18n-Stand vom 2026-06-12 mit 17.593 exportierten Übersetzungen und 0 offenen GUI-Einträgen bestehen; der aktive Workflow-Katalog zählt weiterhin konsistent 71 Vorlagen. Der Daily-Care-Lauf vom 2026-07-03 hat `python bach.py agent doctor test-agent --json` und `python bach.py usecase run 50 --dry-run` erneut verifiziert und zusätzlich die Skill-Source-Registry-Lücke für `bach skills version bach` geschlossen: Der Befehl liest den kanonischen Repo-Root-Stand jetzt über `system/data/skill_sources.json` und kann optionale Codex-/Claude-Nutzerkopien bei Bedarf mitprüfen. Nächste Schritte bleiben tieferes Active-Run-Steering in langlaufenden Scheduler-Innenschleifen, Installer-End-to-End-, GUI-Regressions- sowie die breiteren T01/T02-Retestspuren.
