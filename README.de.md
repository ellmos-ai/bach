...
Stand 2026-07-16: OpenClaw Stable ist `2026.7.1`; das neueste sichtbare Prerelease ist `2026.7.2-beta.1`, veröffentlicht am 2026-07-15 um 18:48 UTC. Die bisherigen Privacy-/Recovery-Signale bleiben relevant; die neue Beta ergänzt Remote-Coding-Session-Platzierung, Cron-Historie aus einem Task-Ledger, session-lokal isolierte MCP-Verbindungen, Worktree-Limits, dauerhaften Channel-Ingress und stärkere Gateway-/Session-Recovery. Für BACH passen besonders MCP-Isolation je Session, ledger-gestützte Scheduler-Historie und explizites Remote-Worker-Routing; breite Mobile-/Channel-Parität bleibt außerhalb des aktuellen Fokus. Quelle: [openclaw/openclaw releases](https://github.com/openclaw/openclaw/releases/tag/v2026.7.2-beta.1).

Auf BACH-Seite hat der Daily-Care-Lauf vom 2026-07-16 die veralteten Geisterdatenbank-Tasks `1163` und `1167` geschlossen, nachdem bestätigt wurde, dass der Task-`1165`-Rollout die verzählten `parents[]`-Runtime-Pfade bereits beseitigt hat. Die AST-Regression steht jetzt bei `63 passed`, blockiert neue `bach.db`-Eigenkonstruktionen und hält die verbleibende reine Tools-Altschuld schrumpfend; `test-agent`-Doctor/Start-Dry-Run und Usecase `50` blieben ebenfalls grün. Nächste DB-Pfad-Slices sind Transitquelle, Importzeit-Konstanten, Notfall-Fallback und Testisolation in den Tasks `1168–1171`.
