...
Stand 2026-07-02: Auf der OpenClaw-Releases-Seite ist `2026.6.11` jetzt die aktuelle Stable-Linie; das neueste sichtbare Prerelease ist `2026.7.1-beta.1`, veröffentlicht am 2026-07-02 um 07:25 UTC. Für BACH sind daraus vor allem GPT-5.6-Modellfamilien-Support, `openclaw attach` für externe Harness-Anbindung an bestehende Sessions, Telegram-Codex-Pairing/Steering/Recovery, exit-getriggerte Cron-Läufe mit sauberem Detach, gescopte Conversation-Capability-Profile sowie deutlichere Doctor-/Install-Diagnostik relevant. Das bestätigt BACHs Fokus auf Codex-/Runtime-Kontrolle, Scheduler-Trigger, Capability-Grenzen und belastbare Recovery-Oberflächen. Quelle: [openclaw/openclaw releases](https://github.com/openclaw/openclaw/releases).

Auf BACH-Seite bleibt der saubere GUI-i18n-Stand vom 2026-06-12 mit 17.593 exportierten Übersetzungen und 0 offenen GUI-Einträgen bestehen; der aktive Workflow-Katalog zählt weiterhin konsistent 71 Vorlagen. Der Daily-Care-Lauf vom 2026-07-02 hat `python bach.py --startup quick --mode=silent --partner=codex`, `python bach.py agent doctor test-agent --json`, `python bach.py agent start test-agent --dry-run --json` und `python bach.py usecase run 50 --dry-run` erneut verifiziert und zusätzlich den Upgrade-Self-Heal-Pfad bestätigt: `python bach.py upgrade repair --version v3.12.4-earth --json` hob einen Live-Drift von `release_entries=0` wieder auf `release_entries=1`, danach meldete `python bach.py upgrade check --json` erneut `repair_recommended=false`. Nächste Schritte bleiben tieferes Active-Run-Steering in langlaufenden Scheduler-Innenschleifen, Installer-End-to-End-, GUI-Regressions- sowie die breiteren T01/T02-Retestspuren.
