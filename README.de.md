...
Stand 2026-06-03: GitHub Releases markiert weiterhin `2026.5.28`, veröffentlicht am 2026-05-30, als aktuelle Stable-Linie von OpenClaw; im offiziellen Release-Feed ist jetzt `2026.6.1-beta.3`, veröffentlicht am 2026-06-03, das neueste sichtbare Prerelease, und die GitHub-Containerseite hebt aktuell `2026.6.1-beta.3-browser` hervor, während `2026.6.1-beta.3-slim` zu den neuesten getaggten Images zählt. Für BACH sind daraus vor allem robustere Recovery nach unterbrochenen Tool-Calls und veralteten Session-Bindings, strengere Timer-/Retry-Grenzen in Provider- und Plugin-Pfaden, klarere Behandlung veralteter deaktivierter Skill-/Plugin-Snapshots, breitere Workboard-/SecretRef-Manifest-Flächen, mehr SQLite-gestützte Install-/Laufzeit-Zustände und stärker begrenzte CI-/Docker-/E2E-Diagnostik relevant. Auf BACH-Seite hat der heutige Live-Check die GUI-i18n-Lücke weiter verkleinert: Fünf noch unindexierte Legacy-Texte aus `system/gui/ki-center.html` wurden in den bestehenden `gui`-Namespace aufgenommen, und die Release-Artefakte wurden auf 17.419 exportierte Übersetzungen aktualisiert. Derselbe Durchlauf hat außerdem `bach agent doctor test-agent --json`, `bach usecase run 12 --dry-run`, `bach usecase run 41 --dry-run`, `bach upgrade check --json` und `bach lang report --surface gui --limit 10 --json` erneut verifiziert; der GUI-Report liegt jetzt bei 148 offenen eindeutigen Einträgen und 192 offenen Fundstellen, während Usecase 41 weiterhin ohne verknüpfte Workflow-Datei im manuellen Fallback landet. Nächste Schritte bleiben tieferes Active-Run-Steering in langlaufenden Scheduler-Innenschleifen, Low-Cardinality-Telemetrie, Installer-End-to-End- sowie GUI-Regressionsabdeckung und die priorisierte Bereinigung der vom `bach lang report` sichtbaren UI-/Hilfe-/Skill-/Tool-Texte.
