...
- **Gated-Spawn-Nachtrag 2026-09-15:** Der BACH-Startpfad hält Windows-`start.bat` per einmaligem Pipe-Token und Unix per providerlosem Python-Starter an, bis `process_create_time` und PID-Beleg geschrieben sind. Birth-Fehler/Abfrageausnahme schließen die Pipe ohne Provider-Freigabe und warten auf Starter-Ende; unbestätigtes Ende bleibt `unverified`. Harmlose Windows-Headless-Gate- und integrierte Handler-Tests bestanden, Unix-Gate-Tests sind auf diesem Windows-Host übersprungen. **Nicht geschlossen:** interaktiver Windows-New-Console-Host-Smoke, Unix-Host-Smoke, Gate-Freigabe-Ack, OS-gehaltener Stop-/Baum-Fence und BACH/Modul/Ocean-Parität.
- **Aufwand:** medium
- **Reichweite:** local
- **Priorität:** high
