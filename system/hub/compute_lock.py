...
Check-script: automatisch aufgeloest (siehe _resolve_check_script)
  Exit 0 = free, Exit 1 = active (JSON on stdout)
...
_CHECK_SCRIPT_CANDIDATES = [
    "~/compute/abc_hct/compute_queue/check_compute_active.sh",
    "~/compute/general_compute/queue/scripts/check_compute_active.sh",
    "~/compute/abc_hct/_compute_queue/check_compute_active.sh",
]
...
def _resolve_check_script() -> str:
    """Ersten existierenden Kandidaten zurueckgeben (Queue-Verzeichnisse variieren)."""
    for cand in _CHECK_SCRIPT_CANDIDATES:
        if _expand(cand).is_file():
            return cand
    return _CHECK_SCRIPT_CANDIDATES[0]
