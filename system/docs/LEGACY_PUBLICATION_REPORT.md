...
- **Pfad:** `%ONEDRIVE_AI_ROOT%\_ARCHIV\_CHIAH\`
- **Dateien:** 322
- **Git-Repo:** Nein (muss initialisiert werden)
- **Version:** 3.2 (final)
- **Beschreibung:** CLI-first System für Claude AI mit Injektoren, Context Sources, Auto-Logging

### recludOS (Reclusive OS)
- **Pfad:** `%ONEDRIVE_AI_ROOT%\_ARCHIV\recludOS\`
- **Dateien:** 5296
- **Git-Repo:** Nein (muss initialisiert werden)
- **Version:** 3.3.0 (final)
- **Beschreibung:** Text-basiertes OS/Framework für LLM AI mit Agent-Framework, Distribution-System, Self-Healing

### recludos-filecommander-mcp (Bonus)
- **Pfad:** `%ONEDRIVE_AI_ROOT%\_ARCHIV\recludos-filecommander-mcp\`
- **Git-Repo:** Nein
- **Beschreibung:** MCP-Server (TypeScript/Node.js) ursprünglich für recludOS entwickelt
...
- **RECLUDOS_ROOT in .py Dateien:** 0 Treffer -- vollständig bereinigt (Task #2)
...
- **"recludos" (case-insensitive) in BACH:** 80 Dateien -- fast ausschließlich Dokumentation, Wiki, Changelogs und historische Referenzen. Dies ist korrekt und sollte erhalten bleiben.
...
- **Potentiell sensibel:** `DATA/memory/archive/` enthält Session-Berichte mit "api-key", "token" Erwähnungen -- Kontextuelle Erwähnungen, keine echten Keys. Trotzdem via `.gitignore` ausgeschlossen.
- **`.gitignore` erstellt:** Schließt `DATA/memory/archive/`, `_sandbox/`, `__pycache__/` aus.

### recludOS
- **Keine echten Credentials gefunden.** "token" Erwähnungen sind kontextuell (Token-Counting, Dokumentation).
- **Potentiell sensibel:** `Workspace/Mail_Downloads/`, PDFs im Workspace.
- **`.gitignore` erstellt:** Schließt `Workspace/Mail_Downloads/`, `Workspace/Papierkorb/`, `node_modules/`, `*.pdf` im Workspace aus.
...
Falls noch nicht auf GitHub: Gleiches Verfahren. Prüfen ob bereits unter `github.com/lukisch` vorhanden.
...
- Wiki-Einträge (`wiki/recludos.txt`, `wiki/chiah.txt`, `wiki/batch.txt`)
- CHANGELOG/ROADMAP historische Einträge
- Architektur-Dokumentation
- `dictionary_builder.py` Variable `RECLUDOS_BASE` (Legacy-Tool, funktional unabhängig)
