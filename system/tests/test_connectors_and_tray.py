...
class TestBACHTray:
    ...
    def test_prompt_failure_shows_the_backend_reason(self, tray):
        ...
        tray._api = MagicMock(side_effect=[{"available": True}, {"ok": False,
    ...
    def test_prompt_without_any_response_still_reports_failure(self, tray):
        tray.icon = MagicMock()
        tray.state.update(connected=True, backend_available=True)
        tray._api = MagicMock(side_effect=[{"available": True}, None])
        tray._send_prompt("Was geht?")
    ...
    @pytest.mark.parametrize("body, expected", [
        ({"ok": False, "answer": "Backend-Fehler: ReadTimeout"}, {"ok": False, "answer": "Backend-Fehler: ReadTimeout"}),
        ({"unexpected": True}, None),
    ])
    def test_api_distinguishes_confirmed_http_failure_from_unknown_outcome(self, tray, body, expected):
        import io
        error = urllib.error.HTTPError("http://localhost/api/chat", 502, "Bad Gateway", {},
                                       io.BytesIO(json.dumps(body).encode()))
        with patch("urllib.request.urlopen", side_effect=error):
            assert tray._api("POST", "/api/chat", {"prompt": "Test"}) == expected
...
from hub._services.chat.chat_tray import _instance_lock_path
print(_instance_lock_path())
"""
        ...
        for temp_dir in session_tmpdirs:
            ...
            env.update({
                ...
                "TEMP": str(temp_dir),
                "TMP": str(temp_dir),
                "BACH_RUNTIME_DIR": str(home / "runtime"),
            })
        ...
        assert lock_paths == [home / "runtime" / "chat_tray.instance.lock"] * 2
