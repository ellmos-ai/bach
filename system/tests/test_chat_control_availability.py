...
def test_control_http_preserves_failure_and_compute_lock_contract(control_module, monkeypatch, outcome):
    backend = OllamaBackend(default_model="test")
    monkeypatch.setattr(backend, "availability", lambda **kwargs: (True, "bereit"))
    control_module.runtime.backend = backend
    control_module.runtime.sessions.clear()
    if outcome == "locked":
        process = AsyncMock(side_effect=control_module.ComputeLocked("Compute-Lock aktiv"))
    else:
        process = AsyncMock(return_value=control_module.FailedAnswer("Backend-Fehler: ReadTimeout"))
    ...
    thread.start()
    try:
        response = httpx.post(f"http://127.0.0.1:{server.server_port}/api/chat",
                              json={"prompt": "Aufgabe", "chat_id": "test"}, timeout=3)
    finally:
        server.shutdown()
        server.server_close()
        thread.join(timeout=2)
    ...
    assert response.status_code == (200 if outcome == "locked" else 502)
    assert payload.get("compute_locked", False) is (outcome == "locked")
    assert process.call_args.kwargs["backend"] is backend


def test_compute_lock_uses_selected_ollama_after_default_switch(control_module, monkeypatch):
    selected = OllamaBackend(default_model="selected")
    control_module.runtime.backend = SimpleNamespace()
    monkeypatch.setattr(control_module, "HAS_COMPUTE_LOCK", True)
    monkeypatch.setattr(control_module, "CONFIG", {"compute_lock": {"enabled": True}})
    check = MagicMock(return_value=(True, {}))
    monkeypatch.setattr(control_module, "check_compute_active", check)
    assert control_module._compute_lock_blocks(selected) is True
