...
class TestBatScript:
    ...
    def test_sets_delayed_expansion(self):
        assert any("enabledelayedexpansion" in l.lower() for l in self.lines[:5])

    def test_has_menu_label(self):
        assert any(l.strip() == ":menu" for l in self.lines)

    def test_has_end_label(self):
        assert any(l.strip() == ":end" for l in self.lines)
    ...
    def test_chat_dir_reference(self):
        ...
        assert "telegram_chat.py" in self.text

    def test_gui_server_reference(self):
        assert "gui\\server.py" in self.text or "gui/server.py" in self.text

    def test_control_api_port(self):
        assert "8081" in self.text
...
class TestCrossScriptConsistency:
    ...
    def test_both_reference_control_api_port(self):
        assert "8081" in self.bat
        assert "8081" in self.sh

    def test_both_reference_gui_port(self):
        assert "8000" in self.bat
        assert "8000" in self.sh

    def test_both_reference_chat_tray(self):
        assert "chat_tray.py" in self.bat
        assert "chat_tray.py" in self.sh

    def test_both_reference_telegram_chat(self):
        assert "telegram_chat.py" in self.bat
        assert "telegram_chat.py" in self.sh
    ...
    def test_both_reference_gui_server(self):
        assert "server.py" in self.bat
        assert "server.py" in self.sh
