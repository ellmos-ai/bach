...
# T-20260913-709822598: `now` wird per _iso_now strikt normalisiert (immer 26
# Zeichen, %Y-%m-%dT%H:%M:%S.%f). Die fruehere 'T1'/'T2'/'T3'-Platzhalter-
# Konvention (now wurde unveraendert durchgereicht) wirft seither ValueError --
# die Tests nutzen deshalb gueltige ISO-Zeitstempel; *_NORM ist die Form, die
# nach der Normalisierung in der DB landet.
NOW = "2026-01-01T00:00:00"
NOW_NORM = "2026-01-01T00:00:00.000000"
NOW2 = "2026-01-02T03:04:05"
NOW3 = "2026-01-03T06:07:08"
...
class TestNoOp:
    ...
    def test_same_value_again_updates_row_but_writes_no_history(self, conn):
        existing = _row(conn)
        changed = apply_task_field_changes(conn, 1, existing, {"status": "open"}, now=NOW)
        assert changed is True  # UPDATE laeuft trotzdem (updated_at), nur keine History-Zeile
        assert _history(conn) == []
        assert _row(conn)["updated_at"] == NOW_NORM


class TestStatusChangeHistory:
    def test_status_change_is_marked_as_status_change(self, conn):
        apply_task_field_changes(conn, 1, _row(conn), {"status": "in_progress"}, now=NOW)
        rows = _history(conn)
        assert len(rows) == 1
    ...
    def test_non_status_field_is_marked_as_field_change(self, conn):
        apply_task_field_changes(conn, 1, _row(conn), {"priority": "P1"}, now=NOW)
        rows = _history(conn)
        assert len(rows) == 1
    ...
    def test_changed_by_default_and_override(self, conn):
        ...
        apply_task_field_changes(
            conn, 1, _row(conn), {"priority": "P4"}, changed_by="cli-task", now=NOW2
        )
...
class TestStartedAt:
    def test_in_progress_sets_started_at_once(self, conn):
        apply_task_field_changes(conn, 1, _row(conn), {"status": "in_progress"}, now=NOW)
        assert _row(conn)["started_at"] == NOW_NORM

        # Rueckfall und erneutes in_progress darf den Erststart nicht ueberschreiben.
        apply_task_field_changes(conn, 1, _row(conn), {"status": "open"}, now=NOW2)
        apply_task_field_changes(conn, 1, _row(conn), {"status": "in_progress"}, now=NOW3)
        assert _row(conn)["started_at"] == NOW_NORM
...
class TestCompletedVocabulary:
    ...
    def test_either_value_sets_completed_at(self, conn, status_value):
        apply_task_field_changes(conn, 1, _row(conn), {"status": status_value}, now=NOW)
        assert _row(conn)["completed_at"] == NOW_NORM
...
class TestClearFields:
    ...
    def test_clear_fields_sets_column_to_null(self, conn):
        apply_task_field_changes(conn, 1, _row(conn), {"status": "done"}, now=NOW)
        assert _row(conn)["completed_at"] == NOW_NORM

        # done -> pending ist seit T-20260916-1330 ein guarded Reopen (der Task
        # traegt completed_at und gilt damit als terminal geparkt) -- der echte
        # CLI-Aufrufer (task.py _reopen) uebergibt allow_reopen=True.
        apply_task_field_changes(
            conn, 1, _row(conn), {"status": "pending"},
            clear_fields=("completed_at",), now=NOW2, allow_reopen=True,
        )
        assert _row(conn)["completed_at"] is None

    def test_clear_fields_writes_field_change_history_when_value_was_set(self, conn):
        apply_task_field_changes(conn, 1, _row(conn), {"status": "done"}, now=NOW)
        # done -> pending: guarded Reopen, wie der echte task.py-_reopen-Aufrufer
        apply_task_field_changes(
            conn, 1, _row(conn), {"status": "pending"},
            clear_fields=("completed_at",), changed_by="cli-task", now=NOW2,
            allow_reopen=True,
        )
        rows = _history(conn)
        clear_row = next(r for r in rows if r["field_changed"] == "completed_at")
        assert clear_row["action"] == "field_change"
        assert clear_row["old_value"] == NOW_NORM
    ...
    def test_clear_fields_no_history_row_when_already_null(self, conn):
        ...
        apply_task_field_changes(
            ...
            clear_fields=("completed_at",), now=NOW,
        )
        rows = _history(conn)
    ...
    def test_clear_fields_alone_without_field_values_is_not_a_no_op(self, conn):
        ...
        changed = apply_task_field_changes(
            conn, 1, _row(conn), {}, clear_fields=("completed_at",), now=NOW2
        )
        assert changed is True
        assert _row(conn)["completed_at"] is None

    def test_clearable_columns_are_the_timestamp_columns(self):
        # T-20260913-709822598: claimed_by/claimed_at sind seit dem atomaren
        # Claim zusaetzlich CLEARABLE (release_claim setzt beide auf NULL, damit
        # ein freigegebener Task wieder claimbar ist) -- der Terminal-Park-Guard
        # T-20260916-1330 verlaesst sich auf genau dieses Clearing.
        assert CLEARABLE_COLUMNS == frozenset(
            {"started_at", "completed_at", "claimed_by", "claimed_at"})
...
class TestTerminalParkGuard:
    ...
    def test_allow_reopen_bypasses_guard(self):
        ...
        changed = apply_task_field_changes(c, 1, self._row(status="blocked"),
            {"status": "pending"}, now=NOW, allow_reopen=True)
        assert changed is True
