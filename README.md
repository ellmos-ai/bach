...
As of 2026-05-15, GitHub Releases marks `2026.5.12` as OpenClaw's latest stable release and shows `2026.5.14-beta.1` as the latest visible pre-release, while the GHCR versions page already lists `2026.5.14-beta.2-slim`; the `latest` stable container line remains on `2026.5.12-slim`, and the package landing page still trails on `2026.5.14-beta.1-slim`. The newest official notes shift attention less toward channel breadth and more toward operator control and release hardening: provider/plugin dependency externalization, ACP fallback backends, SecretRef-only provider credentials, Windows `USERPROFILE` sandbox blocking, provider-auth flag pass-through, startup trace attribution, mid-run steering via `/queue steer`, explicit gateway scopes for browser control, heartbeat metadata on agent events, guaranteed main-session creation before first agent-to-agent sends, and package-installed Docker validation lanes. BACH already covers adjacent control-plane territory with manifest-first metadata, fail-closed setup checks, broader install scanning for skills/MCP/plugins, privacy-preserving secret/reference handling, memory/wiki provenance views, canonical `bach path` surfaces, machine-readable agent/scheduler status surfaces, workspace-scoped agent-runtime cache invalidation, stable Windows agent tracking via durable console PIDs, `bach agent doctor`, and `bach scheduler doctor` / `bach scheduler session doctor` for operator-facing launch diagnostics, plus chain-level operator controls via `bach chain pause`, `bach chain resume`, and `bach chain steer` at safe llmauto checkpoints. Next high-leverage steps remain extending comparable steering semantics to broader agent/scheduler runs, adding low-cardinality telemetry and release-validation lanes, tightening auth gates, and deciding which backend fallback semantics fit BACH's multi-partner model.
