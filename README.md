...
- **Agent Steering Queue Controls** - `bach agent steer/clear-steer [name] [--json]` lets operators queue or clear mid-run hints without manual temp-file cleanup
...
- **Machine-readable Control & Status Surfaces** - `bach agent list/start/stop/steer/clear-steer/status --json`, `bach scheduler status/jobs/session status --json`, and `bach scheduler session pause/resume/steer/clear-steer --json`, including queue state, timestamps, and operator-note metadata for automation-safe polling
- **Upgrade Drift Check** - `bach upgrade --check` now separates up-to-date files, upgradeable older versions, local drift, and missing tracked files, and degrades cleanly when the distribution DB has no version rows yet
...
As of 2026-05-18, GitHub Releases still marks `2026.5.12` as OpenClaw's latest stable release, while the newest visible pre-release is now `2026.5.16-beta.7` from 2026-05-18 08:47. The GitHub package versions page currently tops out at `2026.5.16-beta.6-slim` plus matching `-arm64` and `-amd64` variants from the same day; this suggests the container registry is still one beta behind the releases feed. The newest official notes keep shifting attention away from raw channel breadth and toward operator control, plugin contracts, QA parity, and safer runtime boundaries: typed `openclaw plugins build/validate/init` workflows, browser dialog state in snapshots, runtime-parity and tool-coverage QA lanes, approval-denial benchmark scenarios for personal agents, localized onboarding/setup flows, forwarded provider-auth flags in the setup wizard, config-gated warm caching for resolved skills, scheduled-run fallback semantics for subagents, per-agent Codex MCP scoping plus default approval modes, malformed extension metadata rejection, MIME sniffing before agent-visible file ingestion, stronger Windows sandbox blocking under `USERPROFILE`, config persistence that ignores malformed stored state, package-installed Docker validation lanes for onboarding and plugin user journeys, runtime-neutral container apt-package args, startup cost attribution during gateway restarts, and browser control that explicitly asks only for the existing `operator.admin` gateway scope. BACH already covers adjacent control-plane territory with manifest-first metadata, fail-closed setup checks, broader install scanning for skills/MCP/plugins, privacy-preserving secret/reference handling, memory/wiki provenance views, canonical `bach path` surfaces, machine-readable agent/scheduler status surfaces, workspace-scoped agent-runtime cache invalidation, stable Windows agent tracking via durable console PIDs, `bach agent doctor`, `bach scheduler doctor` / `bach scheduler session doctor`, machine-readable `bach agent start/stop/steer/clear-steer/status --json` responses for automation-safe operator control, newly machine-readable `bach scheduler session pause/resume/steer/clear-steer --json` replies with queue and timestamp snapshots, queue visibility through `pending_operator_notes`, `latest_operator_note`, `latest_steer_message`, and `latest_steer_requested_at`, per-agent launch guardrails through `permission_mode`, `allowed_tools`, and `max_turns`, deprecation-aware session doctor/status surfaces for the legacy pyautogui automation, chain-level operator controls through `bach chain pause`, `bach chain resume`, and `bach chain steer` at safe llmauto checkpoints, and `bach upgrade --check` as a first-class drift surface for versioned distribution files. Next high-leverage steps remain extending comparable steering semantics to broader agent/scheduler runs, tightening per-agent tool/connector scopes, hardening ingest validation and malformed-state cleanup, adding low-cardinality telemetry and release-validation lanes, formalizing i18n drift checks, and replacing deprecated GUI-injection automation with supported background delegation paths.
