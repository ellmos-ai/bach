...
As of July 13, 2026, the OpenClaw releases page still marks stable `2026.6.11` as the latest release, while the newest visible prerelease is now `2026.7.1-beta.6`, published on July 13, 2026 at 01:38 UTC. The earlier signals remain relevant for BACH: GPT-5.6 model-family coverage, `openclaw attach`, Telegram Codex pairing/steering/recovery, exit-triggered cron runs, scoped conversation capability profiles, approval-guided onboarding, ClawRouter model/budget discovery, and crash-loop recovery. Beta 6 adds especially relevant hardening around process-local SecretRef sentinels, token redaction across log chunks, bounded provider/network responses, migrations before readiness, and reliable hard-timeout/subagent completion states. The strongest fits for BACH are privacy-safe credential boundaries and recovery-safe runtime/scheduler surfaces. Source: [openclaw/openclaw releases](https://github.com/openclaw/openclaw/releases/tag/v2026.7.1-beta.6).

On the BACH side, the July 13 care pass independently certified and closed task `1165`: roughly 70 runtime, agent, connector, GUI, and service surfaces now resolve the live database through `hub.bach_paths`, and the central regression prevents new path reconstruction while tracking the remaining legacy debt. The focused suite completed with `61 passed`; `test-agent` doctor/start dry-run and usecase `50` also remained green. The upgrade check correctly identifies that `v3.13.0-bluesky` is not yet registered in the distribution catalog (`repair_recommended=true`), so sealing is deferred until the manifest and its privacy-sensitive historical paths are cleaned. Next DB-path slices are the explicit fallback, transit-source, import-time constant, and test-isolation tasks `1166–1171`.
