...
As of 2026-06-21, the OpenClaw releases page now shows stable `2026.6.9` on the June 21, 2026 release line (with release assets and source archives stamped between `2026-06-21T01:16:53Z` and `2026-06-21T03:57:38Z`), while the newest visible prerelease is `2026.6.10-beta.1`, published on June 21, 2026 at 09:12 UTC. The most relevant fresh signals for BACH are richer Telegram/Slack/Discord delivery, more dependable agent/session recovery, stronger Codex approval and plugin flows, standalone provider-plugin packaging, explicit search/skill provenance, safer release/network boundaries, and clearer CLI/status workflows. Source: [openclaw/openclaw releases](https://github.com/openclaw/openclaw/releases).

On the BACH side, the clean GUI i18n state from 2026-06-12 remains intact at 17,593 exported translations with 0 missing GUI entries, and the active workflow catalog still resolves consistently to 71 templates. The 2026-06-21 care pass hardens ProSync startup recovery: `system/hub/db_sync.py` now stages OneDrive transit backups locally and defers a failing backup candidate for about 30 minutes after cloud-timeout or `disk I/O` errors, so one unreadable `.bachdb` file no longer stalls every CLI or `--json` call. Verification re-ran `python -m pytest system/tests/test_db_sync_handler.py -q` (`45 passed`), `python -m pytest system/tests/test_prosync_race.py -q` (`3 passed`), reproduced a real OneDrive timeout once via `sync_on_start()`, and then re-validated `python bach.py agent doctor test-agent --json`, `python bach.py --startup quick --mode=silent --partner=codex`, `python bach.py usecase run 50 --dry-run`, `python bach.py upgrade check --json`, `python bach.py task list --filter clutch`, and the full `test-agent` control cycle, all green. Next high-leverage steps remain deeper active-run steering inside long-running scheduler internals, installer end-to-end coverage, GUI regression coverage, and the broader T01/T02 retest lanes.
