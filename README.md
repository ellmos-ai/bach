...
As of 2026-05-30, GitHub Releases marks `2026.5.27`, published on May 28, 2026 at 11:41 UTC, as OpenClaw's latest stable release; the newest visible prerelease on the official release feed is `2026.5.28-beta.4`, published on May 29, 2026 at 22:48 UTC, and the GitHub container page currently foregrounds `2026.5.28-beta.4-slim`. The latest official signals most relevant to BACH are steadier agent/Codex runtime recovery and session-lock cleanup, stricter browser/channel/automation input validation, hotter plugin/Gateway caches on hot paths, clearer doctor/auth recovery, bounded release/E2E waits, and new workboard-style agent handoff surfaces. On the BACH side, today's live care pass closed the remaining release-catalog gap: `bach upgrade repair` now bootstraps the current `distribution_releases` entry when the catalog is empty, and `bach upgrade check --json` now surfaces `manifest_entries`, `release_entries`, `repair_recommended`, `current_version`, and `current_release_registered` consistently even outside the empty-db path. The same pass revalidated `bach --startup quick --mode=silent --partner=codex`, `bach task list`, `bach agent doctor test-agent --json`, the full `test-agent` headless operator-control loop (`steer` -> `start` -> `status` -> `pause` -> `checkpoint` -> `resume` -> `stop` -> `clear-steer`), `usecase run-all --dry-run`, `usecase run 12/41/45 --dry-run`, `bach lang report --surface gui --limit 5 --json`, targeted pytest for `upgrade`, and the repaired `upgrade status/check --json` path now reporting 4,720 tracked files, 4,722 manifest entries, 1 registered stable release, and 12 local modifications. Next high-leverage steps remain deeper active-run steering inside long-running scheduler internals, low-cardinality telemetry, installer end-to-end coverage, GUI regression coverage, and prioritized i18n cleanup across GUI, help, skills, and tools.
