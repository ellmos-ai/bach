...
As of July 11, 2026, the OpenClaw releases page still marks stable `2026.6.11` as the latest release, while the newest visible prerelease is now `2026.7.1-beta.5`, published on July 11, 2026 at 10:36 UTC. The earlier signals remain relevant for BACH: GPT-5.6 model-family coverage, `openclaw attach`, Telegram Codex pairing/steering/recovery, exit-triggered cron runs, scoped conversation capability profiles, and more actionable doctor/install diagnostics. The newest beta now adds conversational onboarding with model-judged approvals, a bundled ClawRouter provider with credential-scoped model discovery plus usage/budget reporting, safer Gateway crash-loop recovery, and stronger session organization. The strongest fits for BACH are approval-guided setup, provider/model discovery with budget visibility, and recovery-safe runtime/scheduler surfaces. Source: [openclaw/openclaw releases](https://github.com/openclaw/openclaw/releases).

On the BACH side, the clean GUI i18n state from 2026-06-12 remains intact at 17,593 exported translations with 0 missing GUI entries, and the active workflow catalog still resolves consistently to 71 templates. The 2026-07-11 care pass closed the runtime compat slice for task `1154`: `clutch`-backed adapters now cover route analysis, gas/brake strategy, and the bordcomputer health surface while preserving BACH signatures and the canonical `bach.db` path, so `bach clutch migration` now reports both `Compat-Adapter: OK` and `DB-Brücke: OK` while fork archiving stays intentionally blocked. The same pass re-verified `python -m pytest system/tests/test_delegation_adapter.py system/tests/test_clutch.py system/tests/test_partner_handler.py -q`, `python bach.py partner delegate "Externe clutch Adapter fuer Streckenanalyse, Gas Bremse und Bordcomputer pruefen" --score`, and `python bach.py clutch analyse "Externe clutch Adapter fuer Streckenanalyse, Gas Bremse und Bordcomputer pruefen"`. Next high-leverage steps remain deeper active-run steering inside long-running scheduler internals, installer end-to-end coverage, GUI regression coverage, and the broader T01/T02 retest lanes.
