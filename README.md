...
As of 2026-06-23, the OpenClaw releases page still marks stable `2026.6.9` as the latest release, while the newest visible prerelease is `2026.6.10-beta.2`, published on June 22, 2026 at 09:36 UTC. The most relevant fresh signals for BACH are automatic fast-mode handling for short turns, more reliable model routing, safer session/channel state, trusted-policy preservation across hook composition, richer Telegram/Slack/Discord delivery, more dependable agent/session recovery, stronger Codex approval and plugin flows, explicit search/skill provenance, and safer release/network boundaries. Source: [openclaw/openclaw releases](https://github.com/openclaw/openclaw/releases).
