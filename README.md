...
As of 2026-06-01, GitHub Releases marks `2026.5.28`, published on May 30, 2026, as OpenClaw's latest stable release; the newest visible prerelease on the official release feed is `2026.5.31-beta.1`, published on May 31, 2026, and the GitHub container page currently foregrounds `2026.6.1-beta.1-browser` while also listing `2026.6.1-beta.1-slim` among the newest tagged images. The latest official signals most relevant to BACH remain steadier agent/Codex runtime recovery and session-lock cleanup, stricter browser/channel/automation input validation, hotter plugin/Gateway caches on hot paths, clearer handling for stale disabled skill/plugin snapshots, and broader workboard/SecretRef-manifest handoff surfaces. On the BACH side, today's live care pass pushed more remaining path stragglers onto the canonical `BACH_DB` surface: `system/tools/mcp_server.py` now uses the shared canonical database, the DB viewer prefers `BACH_DB`/`~/.bach/bach.db`, tax helper tooling stops assuming legacy OneDrive roots, and help texts now name `BACH_DB` instead of `system/data/bach.db`. The same pass revalidated `python -m py_compile` on the touched runtime files, `python -m pytest system/tests/test_bach_paths.py -q` (`56 passed`), `beleg_vorfilter.py --dry-run`, `bach agent doctor test-agent --json`, the full `test-agent` headless operator-control loop (`steer` -> `start` -> `status` -> `pause` -> `checkpoint` -> `resume` -> `stop` -> `clear-steer`), `usecase run 12/41`, `usecase run-all --dry-run`, `bach upgrade status/check --json`, and `bach lang report --surface gui --limit 5 --json`, with the live upgrade status now reporting 4,720 tracked files, 4,722 manifest entries, 1 registered stable release, and 52 local modifications. Next high-leverage steps remain deeper active-run steering inside long-running scheduler internals, low-cardinality telemetry, installer end-to-end coverage, GUI regression coverage, and prioritized i18n cleanup across GUI, help, skills, and tools.
