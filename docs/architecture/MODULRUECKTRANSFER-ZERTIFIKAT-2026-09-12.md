...
3. **Plan §4, Regel 3 (Plattformparität):** Der **echte 3-Host-OneDrive-Lauf (Stufe 7, „Offener Betriebs-Nachlauf") ist ab 2026-09-15 abgeleistet** (Task #1244 / Gate3, s. §5, Evidenz `oprun_gate3_2026-09-15.json`, 16/16 PASS). **Noch offen:** Windows-Gegenproben (WORKSTATION-LG) für Stufen 2, 3, 5, 7 und assistant-core-Pin-Konformität aller Hosts. Vor hergestellter Parität keine Ablösung (Grundsatz 2: „Gatung vor Ablösung").
...
- [x] **Echter 3-Host-Lauf sqlite-transit-sync über OneDrive-Transit (Stufe-7-Nachlauf) — ERLEDIGT 2026-09-15** (Task #1244 / TRANSFER-09 Gate3 „Offener Betriebs-Nachlauf"). Evidenz: `system/scripts/transit_sync_3host_oprun.py` lief über die Produktiv-Seam `hub.transit_sync_provider.create_external_engine` mit dem echten Modul `ellmos sqlite-transit-sync` (`module=sqlite_transit_sync`, **nicht** Legacy) auf den 3 Hosts `WORKSTATION-LG` / `ASUS-GEI` / `mac-studio`. 16/16 Checks PASS (ERGEBNIS PASS, EXIT 0). Bewiesen: (1) 3 unabhängige Nodes je mit eigener DB + eigenem Merge-State **OUTSIDE** Transit (Hardinvariante pro Host), (2) 3-Wege-Merge (alle Seeds 10/11/12 konvergent auf jedem Node), (3) LWW-Konfliktlösung (neuerer Timestamp gewinnt: `konflikt-ASUS-GEI`), (4) State-Gate (Re-Pull = No-op, kein Replay), (5) Ausschluss von `secrets` aus Snapshots (secrets=0, öffentlich=1), (6) SyncConfig verweigert State IM Transit (ValueError), (7) Produktionsschnittstelle mac-studio `!= None` auf echtem Transit `~/OneDrive/.SYNC/bach_db_transit` + echtem State-Pfad `~/.bach/transit_sync_state/state.json`. Nicht-destruktiv: Lauf im datierten Subfolder `.oprun-gate3-2026-09-15/`, Produktionssnapshots unangetastet. Merge-State (echter Produktivpfad mac-studio) verifiziert: `last_pulled` enthält `ASUS-GEI` + `WORKSTATION-LG` mit frischen Timestamps (20260915T084917…Z), `protocol=1`. Evidenzdatei: `~/.bach/transit_sync_state/oprun_gate3_2026-09-15.json` (ERGEBNIS PASS). **Offen bleibt** (nicht Teil dieses Gates): Windows-Gegenproben WORKSTATION-LG + assistant-core-Pin-Konformität aller Hosts + Scheduler-Legacy argv-Prämigration + Haltefrist ≥ 2026-10-12.
