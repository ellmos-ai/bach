...
Windows-Headless und Unix starten zunächst providerlos und warten auf einen pro Start eindeutigen Marker, der erst nach PID-Anker und Beleg atomar eingesetzt wird. Der frühere Pipe-Handshake war für interaktive CLI-Eingabe ungeeignet: ein geschlossener Standardeingang hätte sich an den Windows-Provider vererben können. Bei Birth-Fehler beendet der Handler den noch providerlosen Starter ohne Marker; eine unbestätigte Marker-Freigabe wird `unverified` gemeldet. Der Windows-Headless-Pfad wurde mit einem harmlosen Batch und einem echten Handler-Spawn ohne Agent-CLI geprüft. Das beweist **nicht** die interaktive Windows-Konsole, Unix-Hosts, einen gegen fremde lokale Writer gehärteten Marker, einen Freigabe-Ack, einen gehaltenen Stop-Bezug oder dynamische Prozessbaum-Kontrolle. Die Invarianten 2–3 und die Host-/Paritätsprüfungen bleiben offen.
